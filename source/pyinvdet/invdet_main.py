...
def parse_args(argv = None):
    ...
    parser.add_argument("-s", "--start-from", default="begin", choices=["begin", "blasr", "inv-repeats", "extract", "report"], help="start the program from (default: %(default)s)")
...
def main(argv = None):
    ...
    if start_from == "blasr":
        ...
        start_from = "inv-repeats"

    if start_from == "inv-repeats":
        if not args.target_genome:
            logger.critical("-t/--target-genome is missing")
            exit(-1)
        logger.info("Extract inverted repeats using Nucmer")
        ...
        if nthreads <= 1:
            ...
            pool.join()
        # ====== debug stop block =============
        logger.debug("For the purpose of debugging, the program will stop here.")
        exit(0)
        # ====== debug stop block ends ========
